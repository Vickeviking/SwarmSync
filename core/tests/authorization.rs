use common::{APP_HOST, TEST_PASSWORD};
...
async fn test_login() {
    ...
    let username = common::generate_unique_username();
    let client = common::http_client();
    let user: UserResponse = common::create_user_via_api(&client, &username).await;

    // 2) Login: correct password
    let resp = common::login_user(&client, &username, TEST_PASSWORD).await;
    ...
    let resp = common::login_user(&client, &username, "wrong_password").await;
    ...
    common::delete_user_via_api(&authorized_client, user.id).await;
}
